"""
Full Gap Analysis — February 2026
Compares ALL transaction types (expense, income, transfer) in the Excel export
against Plaid rows in Supabase for 2026-02-01 through 2026-02-28.
...
OUTPUT_PATH = ROOT / "data" / "full_gap_analysis_february.csv"
...
excel_df = raw[(raw["Date"] >= FEB_START) & (raw["Date"] <= FEB_END)].copy()

print(f"  File: {EXCEL_PATH.name}")
print(f"  Total February rows: {len(excel_df)}")
print()
...
# ─── STEP 2: Load Plaid transactions from Supabase ───────────────────────────
print("═" * 72)
print("STEP 2 — Loading Plaid transactions from Supabase")
print("═" * 72)
...
plaid_df = all_plaid[
    (all_plaid["date"] >= FEB_START) & (all_plaid["date"] <= FEB_END)
].copy()

print(f"  Plaid rows fetched (Feb 2026): {len(plaid_df)}")
print()
print("  Breakdown by type:")
...
def match_rows(excel_rows: pd.DataFrame, plaid_rows: pd.DataFrame) -> list[dict]:
    """
    Run three-tier matching for a single transaction type.
    Plaid pool is consumed as rows are matched (no double-matching).
    """
    ...
    for _, ex in excel_rows.iterrows():
        ...
        # Tier 2 — exact amount + ±1 day (desc may differ)
        if status is None:
            for idx, pl in candidates.iterrows():
...
for tx_type in ["expense", "income", "transfer"]:
    ...
    print(f"  Matched {tx_type}: {len(ex_subset)} Excel vs {len(pl_subset)} Plaid rows processed")
...
def print_amount_only(df: pd.DataFrame):
    ...
    if ao.empty:
        print("  (none)")
        return
    print(f"  {'Ex Date':<12} {'Pl Date':<12} {'Amount':>8}  {'Excel Description'}")
    print(DIV)
    for _, r in ao.iterrows():
        ex_d = str(r["excel_description"])[:44]
        print(f"  {str(r['excel_date']):<12} {str(r['plaid_date']):<12} ${r['excel_amount']:>7.2f}  {ex_d}")
...
for tx_type in ["expense", "income", "transfer"]:
    ...
    for status in STATUSES:
        ...
        grand_rows  += cnt
        grand_total += total
        marker = "  " if cnt == 0 else "→ " if status == "MISSING - import needed" else "  "
    ...
    print(f"\n  Sanity check (matched + missing == excel total): {'✓ PASS' if check else '✗ FAIL'}")
